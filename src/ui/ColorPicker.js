...
const ColorMath = (() => {
    ...
    const rgbaToHex = ({ r, g, b, a }) => '#' + compHex(r) + compHex(g) + compHex(b) + alphaHex(a);
    ...
    function hsvToRgb(h, s, v) {
        ...
        const c = v * s, x = c * (1 - Math.abs((h / 60) % 2 - 1)), m = v - c; let r1 = 0, g1 = 0, b1 = 0;
        if (0 <= h && h < 60) [r1, g1, b1] = [c, x, 0]; else if (60 <= h && h < 120) [r1, g1, b1] = [x, c, 0];
        else if (120 <= h && h < 180) [r1, g1, b1] = [0, c, x]; else if (180 <= h && h < 240) [r1, g1, b1] = [0, x, c];
        else if (240 <= h && h < 300) [r1, g1, b1] = [x, 0, c]; else[r1, g1, b1] = [c, 0, x];
    ...
    return { clamp, rgbaToHex, hexToRgba, rgbToHsv, hsvToRgb, rgbToHsl, hslToRgb };
})();
...
export class ColorPicker {
    ...
    constructor(container, options = {}) {
        ...
        this.color = { r: 59, g: 130, b: 246, a: 1 }; // Default Blue
     ...
     */
    setColorStr(hexStr) {
        const p = ColorMath.hexToRgba(hexStr);
        if (p) {
            this._updateState(p, false); // 内部からの変更扱いで通知しない、または通知するか検討
            this._render();
        }
    }
     ...
     */
    getColorStr() {
        return ColorMath.rgbaToHex(this.color).toUpperCase();
    }
    ...
    _createDOM() {
        ...
        el.innerHTML = `
            <div class="cp-sv" id="cp-sv" aria-label="Saturation/Value panel">
            ...
            <div class="cp-controls">
                <div class="cp-top">
                    ...
                    <div class="cp-hex">
                        <input class="cp-hex-input" id="cp-hex-input" placeholder="#RRGGBB">
                    </div>
                ...
                <div class="cp-compact">
                    <div class="cp-group" id="cp-inputs">
                        ...
                        <div class="cp-box"><div class="cp-value"><input id="cp-i4" type="number" min="0" max="100"></div><div id="cp-l4" class="cp-label-small">A</div></div>
                    </div>
    ...
    _bindEvents() {
        ...
        // Hex
        const onHex = () => {
            const p = ColorMath.hexToRgba(refs.hexInput.value || '');
            if (p) {
                this._updateState(p);
                this._render();
            } else {
                refs.hexInput.value = ColorMath.rgbaToHex(this.color).toUpperCase();
            }
        };
        ...
        refs.eyeBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (res && res.sRGBHex) {
                    const p = ColorMath.hexToRgba(res.sRGBHex);
                    if (p) { p.a = this.color.a; this._updateState(p); this._render(); }
                }
            } catch (e) {
                // キャンセル等は無視
            }
        ...
        inputs.forEach((el, idx) => {
            ...
            el.addEventListener('change', () => {
                ...
                if (this.currentMode === 'rgba') {
                    ...
                    if (idx === 3) this._updateState({ a: clampInt(val, 0, 100) / 100 });
                } else {
                    // HSLA
                    ...
                    if (idx === 3) na = clampInt(val, 0, 100) / 100;
    ...
    _render() {
        const { color, hsv, refs, currentMode } = this;
        ...
        const hx = ColorMath.rgbaToHex(color).toUpperCase();
        ...
        if (currentMode === 'rgba') {
            ...
            if (active !== refs.i4) refs.i4.value = Math.round(color.a * 100);
            refs.l1.textContent = 'R'; refs.l2.textContent = 'G'; refs.l3.textContent = 'B'; refs.l4.textContent = 'A';
        } else {
            ...
            if (active !== refs.i4) refs.i4.value = Math.round(color.a * 100);
            refs.l1.textContent = 'H'; refs.l2.textContent = 'S'; refs.l3.textContent = 'L'; refs.l4.textContent = 'A';
        }
    }
}
